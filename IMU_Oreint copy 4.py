        ...
        try:
            ...
            for part in parts:
                if len(part) == 44:  # 64-bit timestamp + 9 floats (9 sensor readings)
                    ...
                    # mag1 = mag / np.linalg.norm(mag)

                    # # Calculate heading from magnetometer
                    # heading_mag = np.arctan2(mag1[1], mag1[0])
                    # # Convert heading to degrees
                    # heading_degrees_mag = np.degrees(heading_mag)
                    # if heading_degrees_mag < 0:
                    #     heading_degrees_mag += 360

                    # # Normalize accelerometer and magnetometer data
                    # accel = accel / np.linalg.norm(accel)
                    # # mag2 = mag / np.linalg.norm(mag)
                    ...
                    # mag_x = mag1[0] * np.cos(pitch) + mag1[2] * np.sin(pitch)
                    # mag_y = mag1[0] * np.sin(roll) * np.sin(pitch) + mag1[1] * np.cos(roll) - mag1[2] * np.sin(roll) * np.cos(pitch)
                    ...
                    if q is not None:
                        ...
                        if count == 20:
                            ...
                            # print(f"Heading Mag: {heading_degrees_mag} degrees")
                            print(f"Heading fuse: {heading_degrees_fuse} degrees")
                            count = 0
