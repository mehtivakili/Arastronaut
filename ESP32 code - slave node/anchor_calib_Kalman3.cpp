...
bool isOutlier(float value, float mean, float stddev) {
  return abs(value - mean) > (2 * stddev); // 2 standard deviations for outlier detection
}
...
void newRange()
{
  ...
  if (sampleCount >= sampleSize) {
    ...
    for (int i = 0; i < sampleSize; i++) {
      sum += adelaySamples[i];
      sumSq += adelaySamples[i] * adelaySamples[i];
    }
    float mean = sum / sampleSize;
    float variance = (sumSq / sampleSize) - (mean * mean);
    ...
    sum = 0.0;
    for (int i = 0; i < sampleSize; i++) {
      if (!isOutlier(adelaySamples[i], mean, stddev)) {
        sum += adelaySamples[i];
        validSamples++;
      }
    }

    uint16_t finalAdelay = sum / validSamples; // calculate the average Adelay
