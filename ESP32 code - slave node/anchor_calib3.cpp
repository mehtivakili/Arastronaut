...
float calculateWeightedMovingAverage(float newValue) {
  float weight = 0.9; // Higher weight gives more importance to recent values
  float sum = 0.0;
  float weightedSum = 0.0;
  for (int i = 0; i < movingAverageSize; i++) {
    int index = (distanceSampleCount - 1 - i + movingAverageSize) % movingAverageSize;
    sum += weight * distanceSamples[index];
    weightedSum += weight;
    weight *= 0.9; // Decrease the weight for older samples
  }
  return (sum + newValue) / (weightedSum + 1);
}

void newRange()
{
  static float last_delta = 0.0;
  static float smoothedDist = 0.0;
  Serial.print(DW1000Ranging.getDistantDevice()->getShortAddress(), DEC);

  float dist = DW1000Ranging.getDistantDevice()->getRange();
  distanceSamples[distanceSampleCount % movingAverageSize] = dist;
  distanceSampleCount++;

  smoothedDist = calculateWeightedMovingAverage(dist);
