...
def main():
    ...
    max_time_seconds = 400  # Adjust this value to set the time range for your dataset
    ...
    plt.plot(encoder_df['time_sec'], encoder_df['angle_deg'], linestyle='-', color='black', linewidth=2, alpha=1, label='Full Encoder Angle')

    # Plot Screw Axis Angle (filtered for the time range)
    plt.plot(filtered_screw_axis_df['time_sec'], filtered_screw_axis_df['angle_axis'], linestyle='-', color='red', linewidth=2, label='Screw Axis Angle')

    # Plot Error
    # plt.plot(error_df['time_sec'], error_df['error'], linestyle='-', color='red', label='Absolute Error')

    # Set titles and labels
    plt.title(f'Full Encoder Data, Screw Axis Data', fontsize=16)
