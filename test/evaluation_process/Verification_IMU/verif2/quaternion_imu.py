...
def complementary_filter(accel, gyro, mag, dt, previous_roll, previous_pitch, previous_yaw, alpha=0.00):
...
# Function to unwrap angles to prevent discontinuity when crossing -180 to 180 degrees
...
def data_thread(sock, data_queue, acc_misalignment, acc_scale, acc_bias, gyro_misalignment, gyro_scale, gyro_bias, mag_offsets, mag_scales):
    previous_roll, previous_pitch, previous_yaw = 0.0, 0.0, 0.0
            ...
            for part in parts:
                if len(part) == 44:  # 64-bit timestamp + 9 floats (9 sensor readings)
                    ...
                    if rate == 40:  # Process every 10th packet (adjust rate as needed)
                        ...
                        roll, pitch, yaw = complementary_filter(accel, gyro, mag, dt, previous_roll, previous_pitch, previous_yaw)
