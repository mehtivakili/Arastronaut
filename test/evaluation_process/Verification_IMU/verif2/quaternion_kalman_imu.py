...
# Function to invert a 3x3 matrix (used in Kalman filter)
...
def kalman_filter_update(x, P, z, A, Q, R, H):
    ...
    S_inv = invert_3x3_matrix(S)  # Efficient inversion for 3x3 matrix
    K = P_pred @ H.T @ S_inv  # Kalman gain
    x = x_pred + K @ (z - H @ x_pred)  # Update state with measurements
    P = (np.eye(len(K)) - K @ H) @ P_pred  # Update covariance matrix
