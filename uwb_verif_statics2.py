...
for _, row in window_means.iterrows():
    plt.hlines(
        ...
        colors='orange',
        label='Mean in Each 20-Second Window' if row['Window_Start_Time'] == 0 else "",  # Add label only once
        linestyles='solid',
    )

plt.xlabel('Time (s)', fontsize=12)
plt.ylabel('Distance (m)', fontsize=12)
plt.title('Distance Over Time (With Outliers Highlighted and Constant Mean in 20-Second Windows)', fontsize=14)
...
for idx, row in average_distances_df.iterrows():
    ...
    if window_end_time <= 405:
        gt_distance = 0.4 * (row['Window_Number'] - 1) + 0.4
    else:
        gt_distance = 8.6
...
output_pdf = 'uwb_analysis_plot_with_markers.pdf'
plt.savefig(output_pdf, format='pdf')

print(f"\nFinal analysis plot (with markers and lines) has been saved as '{output_pdf}'.")
plt.show()
