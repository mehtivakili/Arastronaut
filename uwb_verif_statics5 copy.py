...
# -----------------------------
# 14. Generate Scatter Plot: UWB Measurement vs Ground Truth
# -----------------------------

plt.figure(figsize=(16, 8))  # Increased figure size for better readability

# Plot scatter points with red circles ('o') and increased size
plt.scatter(
    average_distances_df['Ground_Truth_Distance (m)'],  # X-axis: Ground Truth
    average_distances_df['Average_Distance (m)'],     # Y-axis: Measured
    color='red',
    marker='o',
...
# Remove NaN values for fitting
scatter_fit_df = average_distances_df.dropna(subset=['Average_Distance (m)', 'Ground_Truth_Distance (m)'])

# Perform linear regression: Measured = slope_scatter * Ground Truth + intercept_scatter
slope_scatter, intercept_scatter = np.polyfit(scatter_fit_df['Ground_Truth_Distance (m)'], scatter_fit_df['Average_Distance (m)'], 1)
linear_fit_scatter = slope_scatter * scatter_fit_df['Ground_Truth_Distance (m)'] + intercept_scatter

# Plot the linear fit
plt.plot(
    scatter_fit_df['Ground_Truth_Distance (m)'],
    linear_fit_scatter,
    linestyle='-',
    color='purple',
...
# Add labels and title with increased font sizes
plt.xlabel('Ground Truth Distance (m)', fontsize=24)  # X-axis: Ground Truth
plt.ylabel('Measured Distance (m) [UWB Measurement]', fontsize=24)  # Y-axis: Measured
...
output_scatter_pdf = 'uwb_vs_ground_truth_scatter_plot.pdf'
plt.savefig(output_scatter_pdf, format='pdf')

print(f"\nScatter plot (UWB Measurement vs Ground Truth) has been saved as '{output_scatter_pdf}'.")
plt.show()

# -----------------------------
# 15. Print the Scatter Linear Fit Equation and RMSE
# -----------------------------

print(f"\nLinear Fit Equation for UWB Measurement Model (Scatter Plot):")
print(f"y = {slope_scatter:.4f}x + {intercept_scatter:.4f} meters")

# Calculate RMSE between regression predictions and actual measured distances
predicted_measured = slope_scatter * scatter_fit_df['Ground_Truth_Distance (m)'] + intercept_scatter
rmse_scatter = np.sqrt(np.mean((scatter_fit_df['Average_Distance (m)'] - predicted_measured) ** 2))

print(f"RMSE of the regression model: {rmse_scatter:.4f} meters")
