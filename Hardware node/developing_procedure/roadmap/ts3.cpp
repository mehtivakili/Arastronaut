...
// Pin Definitions
const uint8_t RST_PIN = 26;    // Reset Pin (ensure different from Tag)
...
void setup() {
    ...
    uint16_t networkId = 0xDECA;     // Common Network ID (must match Tag)
    ...
    attachInterrupt(digitalPinToInterrupt(IRQ_PIN), dw1000_isr, RISING);
