...
<body>
  <div class="config-section">
    ...
    <h1>Serial and WebSocket API Example</h1>
    <button id="connect-ws-button">Connect via WebSocket</button>
    <button id="connect-serial-button">Connect via Serial Port</button>
...
// document.getElementById('connect-ws-button').addEventListener('click', connectWebSocket);
// document.getElementById('connect-serial-button').addEventListener('click', connectSerial);
    ...
    if (totalDataCount > 0 && startTime) {
        ...
        if (lastSecondTimestamp) {
            ...
            if (timeSinceLastSecond >= 1) {
                ...
                if (lastSecondDataCount < 20) {
                    lastSecondDataCount = 20; // Ensure it's always more than 20
                }
                if (lastSecondDataCount < leastDataPerSecondValue) {
                    leastDataPerSecondValue = lastSecondDataCount;
                }
...
function setupSerial() {
    console.log('Serial setup initialized');
}
...
async function connectSerial() {
    ...
    try {
        ...
        statusElement.textContent = 'Status: Connected via Serial';
...
function initializeWebSocket(callback) {
    ...
    ws = new WebSocket(`wss://${location.hostname}:4443/ws`);
...
function connectWebSocket() {
    ...
    ws.onopen = function() {
        statusElement.textContent = 'Status: Connected via WebSocket';
    };
...
function downloadData() {
    const accData = collectedData.map(row => 
        `${parseFloat(row[0]).toExponential(7)} ${parseFloat(row[1]).toExponential(7)} ${parseFloat(row[2]).toExponential(7)} ${parseFloat(row[3]).toExponential(7)}`
    ).join('\n');
    const gyroData = collectedData.map(row => 
        `${parseFloat(row[0]).toExponential(7)} ${parseFloat(row[4]).toExponential(7)} ${parseFloat(row[5]).toExponential(7)} ${parseFloat(row[6]).toExponential(7)}`
    ).join('\n');

    const accCsvContent = "data:text/csv;charset=utf-8," + encodeURIComponent(accData);
    const gyroCsvContent = "data:text/csv;charset=utf-8," + encodeURIComponent(gyroData);

    const accLink = document.createElement("a");
    accLink.setAttribute("href", accCsvContent);
    accLink.setAttribute("download", "acc.csv");
    document.body.appendChild(accLink);

    const gyroLink = document.createElement("a");
    gyroLink.setAttribute("href", gyroCsvContent);
    gyroLink.setAttribute("download", "gyro.csv");
    document.body.appendChild(gyroLink);
