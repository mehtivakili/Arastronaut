...
]) * 10  # Increase Q for accelerometer
...
]) * 10  # Increase Q for gyroscope

# Adjust R to give more weight to the actual measurements
R_acc = np.eye(3) * 0.001  # Lower R for accelerometer
R_gyro = np.eye(3) * 0.001  # Lower R for gyroscope

# Adjust the initial error covariance
acc_P = np.eye(3) * 0.01  # Lower initial P for faster response
gyro_P = np.eye(3) * 0.01  # Lower initial P for faster response
...
time_data = deque(maxlen=200)  # Store the last 1000 data points
acc_data = deque(maxlen=200)
gyro_data = deque(maxlen=200)
        ...
        for part in parts:
            if len(part) == 32:
                ...
                if rate == Rate:
                    ...
                    ax1.plot(time_data, acc_np[:, 0], label='Acc X')
                    ax1.plot(time_data, acc_np[:, 1], label='Acc Y')
                    ax1.plot(time_data, acc_np[:, 2], label='Acc Z')
                    ax1.set_title('Accelerometer Data')
                    ax1.set_xlabel('Time (s)')
                    ax1.set_ylabel('Acceleration')
                    ax1.legend()

                    ax2.plot(time_data, gyro_np[:, 0], label='Gyro X')
                    ax2.plot(time_data, gyro_np[:, 1], label='Gyro Y')
                    ax2.plot(time_data, gyro_np[:, 2], label='Gyro Z')
