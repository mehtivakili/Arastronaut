...
def main():
    ...
    for index, screw_row in filtered_screw_axis_df.iterrows():
        ...
        nearest_encoder_row = filtered_encoder_df.iloc[(filtered_encoder_df['timestamp_ns'] - screw_row['timestamp_ns']).abs().argsort()[:1]]
    ...
    # ----------------------------- Step 8: Plot Screw Axis, Nearest Encoder Data, and Error ----------------------------- #

    # Create a figure
    plt.figure(figsize=(16, 8))

    # Plot Screw Axis Angle
    plt.plot(result_df['time_sec'], result_df['screw_angle'], marker='s', linestyle='-', color='green', label='Screw Axis Angle')

    # Plot Nearest Encoder Angle
    plt.plot(result_df['time_sec'], result_df['encoder_angle'], marker='o', linestyle='-', color='blue', label='Nearest Encoder Angle')

    # Plot Absolute Error
    plt.plot(result_df['time_sec'], result_df['error'], marker='^', linestyle='-', color='red', label='Absolute Error')

    # Set titles and labels
    plt.title(f'Screw Axis, Nearest Encoder Data, and Absolute Error (0-{max_time_seconds}s)', fontsize=16)
